import { GerritEventSnapshot, Workflow, Event } from '@pipeline/types'
...
export const executeWorkflow = (workflow: Workflow, event: GerritEventSnapshot) => {
    const buildContext: {[key: string]: string | number} = {
        buildNumber: 1
    }
    const context: {[key: string]: string} = {}

    const {branchName, revision, projectName, referenceId} = event?.metadata ?? {}
    const eventType = event.type

    console.log("Checking out actions")

    context.displayName = `[${projectName}|${branchName}]: ${eventType}`
    context.description = dedent`
...
//             "PIPELINE_DOCKER_HOST"      : "tcp://docker.infrastructure:2375",
...
//                     withCredentials(bindings: [sshUserPrivateKey(credentialsId: 'gerrit',                                                                                                            keyFileVariable: 'SSH_KEY_FOR_ABC',                                                                                                            passphraseVariable: '',                                                                                                            usernameVariable: '')]) {
...
//         string(name: 'GERRIT_URL', defaultValue: 'ssh://jenkins@gerrit-gerrit-service.infrastructure:29418'),
// ])])
