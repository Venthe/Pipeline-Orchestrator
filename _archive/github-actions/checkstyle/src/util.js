...
async function determinePmdRelease(pmdVersion, token) {
    ...
    const repos = github.getOctokit(token).rest.repos;
    let release;
    if (pmdVersion === 'latest') {
        release = await repos.getLatestRelease({
            owner: 'pmd',
            repo: 'pmd',
        });
    } else {
        release = await repos.getReleaseByTag({
            owner: 'pmd',
            repo: 'pmd',
...
const determineModifiedFiles = async function (token, sourcePath) {
    ...
    if (context.eventName === 'pull_request') {
        return onPull(eventData, octokit, context.repo, sourcePath);
    }

    if (context.eventName === 'push') {
        return onPush(eventData, octokit, context.repo, sourcePath);
    }
...
async function onPull(eventData, octokit, repo, sourcePath) {
...
async function onPush(eventData, octokit, repo, sourcePath) {
...
module.exports.downloadCheckstyle = downloadPmd;
module.exports.executeCheckstyle = executePmd;
