...
describe('process', () => {
  ...
  it('should mask patterns', async () => {
    const result = await shell("echo '--password pass'", {
      output: true,
      mask: [password('--password')]
    });

    expect(result.output?.filter((a) => a.stream === StreamType.STDOUT).map((a) => a.line)).toEqual(
      ['--password *****']
    );
  });

  it('should mask patterns 2', async () => {
    const result = await shell("echo '--password pass2 --password2 p2'", {
      output: true,
  ...
  it('should mask patterns with :', async () => {
    const result = await shell("echo '--password pa:ss'", {
      output: true,
      mask: [password('--password')]
    });
