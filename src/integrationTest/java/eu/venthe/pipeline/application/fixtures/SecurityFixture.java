...
import eu.venthe.pipeline.application.other.jwt.JsonWebToken;
...
import static eu.venthe.pipeline.application.Constants.Keycloak.CLIENT;
import static eu.venthe.pipeline.application.Constants.Keycloak.REALM;
...
public class SecurityFixture {
    ...
    public JsonWebToken login(String username, String password) {
        ...
        JsonNode result = restClient.post().uri(new URIBuilder(container.getAuthServerUrl() + "/realms/" + REALM + "/protocol/openid-connect/token").build())
                ...
                .retrieve()
                .body(JsonNode.class);
        return new eu.venthe.pipeline.application.other.jwt.JsonWebToken(objectMapper, result.get("access_token").asText());
    }
