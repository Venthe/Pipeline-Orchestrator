...
import static eu.venthe.platform.organization.domain.source_configuration.plugins.gerrit.GerritHeaders.getTraceId;
...
public class GerritPluginInstance implements ProjectSourcePluginInstance {
    ...
    @Override
    public Optional<byte[]> getFile(ProjectId projectIdentifier, Revision revision, Path path) {
    ...
    @Override
    public Collection<FileDto> getFileList(ProjectId projectIdentifier, Revision revision, Path path) {
        String string = UriBuilder.fromUri(configuration.getBasePath()).path("/a/").path(projectIdentifier).toString();
        return GitUtilities.onRepository(string, revision, rootDirectory -> {
    ...
    @Override
    public Stream<ProjectId> getProjectIds() {
    ...
    public Optional<ProjectDto> getProject(ProjectId id) {
        ...
        return getProjects()
                .filter(p -> p.getId().equals(new ProjectId(id)))
