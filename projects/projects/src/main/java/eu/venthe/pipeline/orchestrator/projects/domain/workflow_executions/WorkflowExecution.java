...
//    }
    ...
    public WorkflowExecutionStatus getStatus() {
        ...
        if (collect.stream().anyMatch(JobExecutionStatus.CANCELLED::equals)) {
            return WorkflowExecutionStatus.CANCELLED;
        }

        if (collect.stream().anyMatch(JobExecutionStatus.FAILURE::equals)) {
            return WorkflowExecutionStatus.FAILURE;
        }

        if (collect.stream().anyMatch(e -> Set.of(JobExecutionStatus.QUEUED, JobExecutionStatus.IN_PROGRESS, JobExecutionStatus.WAITING).contains(e))) {
            return WorkflowExecutionStatus.IN_PROGRESS;
        }

        if (Set.of(JobExecutionStatus.COMPLETED, JobExecutionStatus.SUCCESS, JobExecutionStatus.SKIPPED, JobExecutionStatus.NEUTRAL).containsAll(collect)) {
