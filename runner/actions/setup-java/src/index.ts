...
(async function () {
  ...
  switch (implementation) {
    case 'corretto':
      ...
      await setupJava(
        type,
        `amazon-corretto-${v}-linux-x64.tar.gz`,
        `amazon-corretto-${v}-linux-x64/bin`,
        `amazon-corretto-${v}-linux-x64`
      );
      break;
    case 'azul':
      ...
      await setupJava(
        type,
        `zulu${v}-linux_x64.tar.gz`,
        `zulu${v}-linux_x64/bin`,
        `zulu${v}-linux_x64`
      );
      break;
    default:
  ...
  if (+versionString <= 5) {
    await shell(
      `keytool -import -trustcacerts -noprompt -storepass changeit -file /certs/ca.crt -keystore ${process.env.JAVA_HOME}/jre/lib/security/cacerts -alias "K8s"`
    );
  } else if (+versionString <= 8) {
    await shell(
      `keytool -importcert -trustcacerts -noprompt -storepass changeit -file /certs/ca.crt -keystore ${process.env.JAVA_HOME}/jre/lib/security/cacerts -alias "K8s"`
    );
  } else {
    await shell(
      'keytool -importcert -trustcacerts -noprompt -storepass changeit -cacerts -file /certs/ca.crt -alias "K8s"'
    );
  }
})();
