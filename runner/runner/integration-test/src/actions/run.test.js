/* eslint-disable @typescript-eslint/no-unused-vars,@typescript-eslint/no-empty-function,@typescript-eslint/no-var-requires */
      ...
      () =>
        test({
          ...
          then: ({ result, log }) => {
            ...
            expect(
              log.filter((l) => l.includes('/:path1/:path2 - Hello to foo/bar !'))[0]
            ).toBeTruthy();
          }
        }),
      timeout
        ...
        test({
          given: () => {
            context.steps = [
              ...
              { run: 'printenv' },
              { run: 'echo "${{ steps.step_1.outputs | stringify }}"' }
            ];
          },
          then: ({ result, log }) => {
            expect(result).toBe('success');
            ...
            test({
              given: () => {
                context.steps = [
                  ...setupJavaSteps(),
                  { uses: 'actions/setup-gradle@main' },
                  { run: `cat /root/.gradle/init.gradle` }
                ];
                withNexusSecret();
              },
              then: ({ result, log }) => {
        ...
        async () =>
          test({
            ...
            then: ({ result, log }) => {
              ...
              assertFilter(log, (line) =>
                line.includes('OpenJDK Runtime Environment Zulu17.38+21-CA (build 17.0.5+8-LTS)')
              );
              assertFilter(log, (line) =>
                line.includes(
      ...
      () =>
        test({
          ...
          then: ({ result, log }) => {
            ...
            assertFilter(log, (line) =>
              line.includes('internal.actionsDirectory: /runner/actions')
            );
            assertFilter(log, (line) =>
              line.includes('internal.pipelinesDirectory: /runner/pipeline')
            );
            assertFilter(log, (line) =>
              line.includes('internal.gerritUrl: ssh://admin@ssh.gerrit.home.arpa:29418')
            );
            assertFilter(log, (line) =>
              line.includes('internal.projectUrl: http://host.docker.internal:')
            );
            assertFilter(log, (line) =>
              line.includes('internal.nexusUrl: https://nexus.home.arpa/repository/raw-hosted')
            );
...
async function test({ given, then }) {
  const run = async (func, params) =>
    typeof func === 'function' ? func?.(params) : await func?.(params);
  ...
  if (context.steps !== undefined) {
    ...
    writeFileSync(
      `${context.repositoryPath}/.pipeline/workflows/${context.workflowName}.yaml`,
      workflow
    );
  } else {
...
function withDockerSecret() {
  writeFileSync(
    `${context.secretsPath}/docker.json`,
    JSON.stringify({
      password: 'secret',
      username: 'admin'
    })
...
function withNexusSecret() {
  writeFileSync(
    `${context.secretsPath}/nexus.json`,
    JSON.stringify({
      password: 'secret',
      username: 'admin'
    })
